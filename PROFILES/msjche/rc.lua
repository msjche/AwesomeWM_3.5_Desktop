...
vicious.register(home_graph, vicious.widgets.fs,'${/home used_p}',2)
...
vicious.register(root_graph, vicious.widgets.fs,'${/ used_p}',2)

data_graph = blingbling.progress_graph({ height = 35,
									width = 60,
									v_margin = 5,
									horizontal = true,
									show_text = true,
...
vicious.register(data_graph, vicious.widgets.fs,'${/media/Data used_p}',2)
...
netwidget = blingbling.net({interface = "eno1",
							...
							graph_color = "#7A5ADA99",
							graph_line_color = "#9F9F9F99",
                			background_graph_color = "#00000033"
							})
...
--    right_layout:add(Default_launcher)
    ...
    left_bottom_layout:add(netdown)
    left_bottom_layout:add(netup)
...
--    awful.key({  }, "F4", function ()
	...
	-- Screencap
	awful.key({ }, "Print", function () awful.util.spawn_with_shell( "~/Scripts/cap.sh") end),
